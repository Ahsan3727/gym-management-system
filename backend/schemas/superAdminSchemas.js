...
const createAdminSchema = z.object({
  ...
  gymName:      z.string().min(1, 'Gym name is required').max(100).regex(/^[^<>]*$/, 'Invalid characters').trim(),
...
const updateAdminSchema = z.object({
  gymName:      z.string().min(1).max(100).regex(/^[^<>]*$/, 'Invalid characters').trim().optional(),
